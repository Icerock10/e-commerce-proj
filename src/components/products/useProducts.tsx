...
import { selectAllProducts, updateProducts } from '../reducers/slices/productsSlice';
...
export const useProducts = () => {
  ...
  useEffect(() => {
    const { categories, headings } = getJSONParsed(t('products', { returnObjects: true }));
    const translatedCategories = products.map((product, index) => ({
      ...product,
      category: categories[index],
      heading: headings[index],
    }));
    dispatch(updateProducts(getLikedProductsFromLocalStorage(translatedCategories)));
  }, [t, dispatch]);

  const productsState = useAppSelector(selectAllProducts);

  const uniqueProducts = [...new Set(productsState.map((item) => item.category))];
